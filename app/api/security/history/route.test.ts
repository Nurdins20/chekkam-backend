...
describe("GET /api/security/history", () => {
  ...
  it("returns 401 when unauthenticated", async () => {
    ...
    const { GET } = await import("@/app/api/security/history/route");
    const res = await GET(new NextRequest("http://localhost/api/security/history"));
    expect(res.status).toBe(401);
  });

  it("scopes a citizen to only their own checks", async () => {
    requireUser.mockResolvedValue({ id: "citizen-1", role: "citizen" });
    const builder = makeQueryBuilder([{ id: "c1", user_id: "citizen-1" }]);
    ...
    expect(body.security_checks).toHaveLength(1);
    expect(builder.eq).toHaveBeenCalledWith("user_id", "citizen-1");
  });

  it("gives staff the full list", async () => {
    requireUser.mockResolvedValue({ id: "staff-1", role: "admin" });
    const builder = makeQueryBuilder([{ id: "c1" }, { id: "c2" }]);
    fromMock.mockReturnValue(builder);
    ...
    expect(res.status).toBe(200);
    expect(body.security_checks).toHaveLength(2);
